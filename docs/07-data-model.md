...
## Identifiers — UUID strings, not ObjectId

Every root document uses a **UUID v4 string** as its `_id` (generated by
`generateId()` in `common/utils`), not a Mongo `ObjectId`. This keeps ids
opaque, portable across stores, and safe to expose in URLs/APIs. Root schemas
declare it explicitly and pull shared options from a base:

```ts
@Prop({ type: String, default: generateId }) _id!: string;
// ...via baseSchemaOptions(strip): timestamps + toJSON { id ← _id, drop _id/__v, strip secrets }
```

So in the schema examples below, **read every `Types.ObjectId` ref as a
`String` ref** to that collection's UUID `_id` — e.g.
`@Prop({ type: String, ref: 'Category' }) parent?: string;`. The `ObjectId`
notation is kept only as shorthand for "a reference to another collection."
API responses surface this as `id` (never `_id`).
