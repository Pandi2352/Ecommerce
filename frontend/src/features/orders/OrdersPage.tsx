...
export function OrdersPage() {
  ...
  const cards: Array<{ label: string; value: ReactNode; tone: StatTone; icon: ReactNode }> = [
    ...
    {
      label: 'Revenue',
      value: s ? formatCurrency(s.revenue) : '—',
      tone: 'emerald',
    ...
    {
      label: 'Avg. order',
      value: s ? formatCurrency(s.avgOrderValue) : '—',
      tone: 'violet',
      icon: <IndianRupee className="size-5" />,
    },
  ];
  ...
  const columns: Column<Order>[] = [
    {
      ...
      cell: (o) => (
        <button type="button" onClick={() => openDetail(o)} className="cursor-pointer text-left">
          <p className="font-mono text-sm font-medium text-text hover:text-indigo-500">
            {o.orderNumber}
          </p>
    ...
    },
    {
      ...
      header: 'Total',
      sortable: true,
      cell: (o) => <span className="whitespace-nowrap text-text">{formatCurrency(o.total)}</span>,
    },
    {
      key: 'status',
      header: 'Status',
      sortable: true,
      cell: (o) => <Badge tone={orderTone[o.status] ?? 'neutral'}>{o.status}</Badge>,
    },
    {
      key: 'paymentStatus',
      header: 'Payment',
      cell: (o) => <Badge tone={payTone[o.paymentStatus] ?? 'neutral'}>{o.paymentStatus}</Badge>,
    },
    {
      key: 'createdAt',
      header: 'Placed',
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          ...
          onChange={(e) =>
            setFilters((f) => ({ ...f, status: e.target.value as OrderStatus | '', page: 1 }))
          }
        >
        ...
        {detail && (
          <div className="space-y-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge tone={payTone[detail.paymentStatus] ?? 'neutral'}>
                {detail.paymentStatus}
              </Badge>
              {detail.paymentMethod && (
                <span className="text-xs text-text-secondary">via {detail.paymentMethod}</span>
              )}
                ...
                {detail.items.map((it, i) => (
                  <div key={i} className="flex items-center justify-between gap-2 px-3 py-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-text-secondary">
                        {it.variant ? Object.values(it.variant).join(' / ') + ' · ' : ''}
            ...
            <div className="space-y-1 text-sm">
              ...
              {detail.shipping > 0 && (
                <Row label="Shipping" value={formatCurrency(detail.shipping)} />
              )}
            ...
            {canWrite &&
              (() => {
                const nextStatuses = nextOrderStatuses(detail.status);
                ...
                return (
                  <div className="rounded-md border border-border p-3">
                    ...
                    {terminal ? (
                      <p className="text-xs text-text-secondary">
                        This order is{' '}
                        <span className="font-semibold text-text">{detail.status}</span> — no
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        >
                          {/* current (note-only) + only valid next transitions */}
                        ...
                        <FormField label="Note (optional)">
                          <Input
                            ...
                            placeholder="e.g. Shipped via BlueDart AWB 123"
                          />
                        </FormField>
                        <Button
                          onClick={applyStatus}
                          loading={statusMutation.saving}
                          disabled={statusValue === detail.status && !note}
                        >
                          Update
                        </Button>
                      </div>
