  ...
  return (
    <Card className="p-5">
      <div className="flex items-center gap-2.5">
        <span className={cn('grid size-8 shrink-0 place-items-center rounded-md', toneChip[tone])}>
          {icon}
        </span>
        <div>
...
export function ProductEditorPage() {
  ...
  const applicableAttrs = useMemo(
    () =>
      attributes.filter(
        (a) =>
          a.isActive !== false &&
          (a.scope === 'all' || (form.category && a.categoryIds.includes(form.category))),
      ),
  ...
  const addOption = () =>
    setForm((f) => ({ ...f, options: [...f.options, { name: '', valuesText: '' }] }));
  ...
  const setVariant = (sig: string, patch: Partial<VariantRow>) =>
    setForm((f) => ({
      ...f,
      variants: f.variants.map((v) => (v.sig === sig ? { ...v, ...patch } : v)),
    }));
  ...
  const renderAttr = (a: AttributeDefinition): ReactNode => {
    ...
    switch (a.type) {
      ...
      case 'number':
        return (
          <FormField key={a.key} label={label} required={a.required}>
            <Input
              type="number"
              value={value === undefined ? '' : String(value)}
              onChange={(e) =>
                setAttr(a.key, e.target.value === '' ? undefined : Number(e.target.value))
              }
            />
          </FormField>
        );
      case 'boolean':
        return (
          <label key={a.key} className="flex items-center gap-2 text-sm text-text">
            <Checkbox checked={!!value} onChange={(e) => setAttr(a.key, e.target.checked)} />
            {label}
          </label>
        );
      case 'select':
                ...
                return (
                  <label key={o} className="flex items-center gap-1.5 text-sm text-text">
                    <Checkbox
                      ...
                      onChange={() =>
                        setAttr(a.key, arr.includes(o) ? arr.filter((x) => x !== o) : [...arr, o])
                      }
                    />
                    {o}
                  </label>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="space-y-4 lg:col-span-2">
            <Section
              ...
              tone="indigo"
              title="Basics"
              description="The essentials shown everywhere."
            >
              <div className="grid grid-cols-2 gap-3">
                ...
                <FormField label="SKU" hint="Your internal code">
                  <Input
                    value={form.sku}
                    onChange={(e) => setForm((f) => ({ ...f, sku: e.target.value }))}
                    placeholder="NOVA-001"
                  />
                </FormField>
            ...
            >
              <Alert tone="info">
                The first image is used as the primary thumbnail in lists and cards.
              </Alert>
              <div className="mt-3">
                <ImageUploader
                  value={form.images}
            ...
            <Section
              ...
              description="Custom attributes for your shop."
            >
              {applicableAttrs.length === 0 ? (
                <Alert tone="info">
                  No custom fields for this {form.category ? 'category' : 'shop'} yet. Define them
                  under <b>Product Fields</b> (Catalog → Product Fields).
                </Alert>
              ) : (
            ...
            <Section
              ...
              tone="amber"
              title="Variants"
              description="Sell size/colour combos with their own stock."
            >
              <Alert tone="info">
                Add options like <b>Size</b> or <b>Color</b>, then <b>Generate</b> to create
                sellable combinations — each with its own SKU, price and stock. No options = a
                simple product using the base price &amp; stock.
              </Alert>
              <div className="mt-3 space-y-2">
                {form.options.map((o, i) => (
                  <div key={i} className="flex items-end gap-2">
                    <FormField label="Option" className="w-40">
                      <Input
                        value={o.name}
                    ...
                    <FormField label="Values (comma-separated)" className="flex-1">
                      <Input
                        value={o.valuesText}
                        onChange={(e) => setOption(i, { valuesText: e.target.value })}
                        placeholder="S, M, L"
                      />
                    </FormField>
                    <Button
                ...
                <div className="flex gap-2">
                  ...
                  {form.options.length > 0 && (
                    <Button variant="secondary" size="sm" onClick={generateVariants}>
                      Generate variants
                    </Button>
                  )}
                </div>
                          ...
                          <tr key={v.sig} className="border-t border-border">
                            <td className="px-3 py-1.5">
                              <div className="flex flex-wrap gap-1">
                                {Object.values(v.optionValues).map((val, k) => (
                            ...
                            <td className="px-2 py-1.5">
                              <Input
                                className="h-8"
                                value={v.sku}
                                onChange={(e) => setVariant(v.sig, { sku: e.target.value })}
                              />
                            </td>
                            <td className="px-2 py-1.5">
                              <Input
                                ...
                                min="0"
                                value={v.price}
                                onChange={(e) => setVariant(v.sig, { price: e.target.value })}
                              />
                            </td>
                            <td className="px-2 py-1.5">
                              <Input
                                ...
                                min="0"
                                value={v.stock}
                                onChange={(e) => setVariant(v.sig, { stock: e.target.value })}
                              />
                            </td>
                          </tr>
                        ))}
          ...
          <div className="space-y-4">
            <Section icon={<Info className="size-4" />} tone="slate" title="Status">
              ...
              <p className="mt-2 text-[11px] text-text-secondary">
                Only <b>Active</b> products appear on the storefront. Use <b>Draft</b> while
                preparing.
              </p>
            </Section>
            ...
            <Section icon={<IndianRupee className="size-4" />} tone="emerald" title="Pricing">
              <div className="space-y-3">
                ...
                <FormField label="Compare-at price" hint="Shown struck-through as the 'was' price">
                  <Input
                    type="number"
                    min="0"
                    step="0.01"
            ...
            <Section icon={<Boxes className="size-4" />} tone="amber" title="Inventory">
              ...
              {form.variants.length > 0 && (
                <p className="mt-2 text-[11px] text-amber-600">
                  This product has variants — stock is tracked per variant, not here.
                </p>
              )}
            </Section>
