...
export function ProductsPage() {
  ...
  const runBulk = (
    action: 'delete' | 'setStatus' | 'feature' | 'unfeature',
  ...
  const cards: Array<{
    ...
    },
    {
      ...
      onClick: () => applyStatus(ProductStatus.ACTIVE),
      active: filters.status === ProductStatus.ACTIVE,
    },
    {
      label: 'Draft',
      value: s?.draft ?? 0,
      ...
      onClick: () => applyStatus(ProductStatus.DRAFT),
      active: filters.status === ProductStatus.DRAFT,
    },
    {
      label: 'Archived',
      value: s?.archived ?? 0,
      ...
      onClick: () => applyStatus(ProductStatus.ARCHIVED),
      active: filters.status === ProductStatus.ARCHIVED,
    },
    {
      label: 'Low stock',
      value: s?.lowStock ?? 0,
  ...
  const stockBadge = (n: number) =>
    n === 0 ? (
      ...
      <Badge tone="warning">Low · {n}</Badge>
    ) : (
  ...
  const columns: Column<Product>[] = [
      ...
      : []),
    {
      ...
      cell: (p) => (
        <button
          type="button"
          onClick={() => canWrite && navigate(`/products/${p.id}/edit`)}
          className={cn(canWrite && 'cursor-pointer text-left hover:text-indigo-500')}
        >
    ...
    },
    {
      key: 'category',
      header: 'Category',
      cell: (p) => (p.category ? (categoryName[p.category] ?? '—') : '—'),
    },
    {
      key: 'price',
      header: 'Price',
    ...
    { key: 'stock', header: 'Stock', sortable: true, cell: (p) => stockBadge(p.stock) },
    {
      ...
      header: 'Status',
      sortable: true,
      cell: (p) => <Badge tone={statusTone[p.status] ?? 'neutral'}>{p.status}</Badge>,
    },
    {
      key: 'actions',
      header: '',
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          ...
          onChange={(e) => applyStatus(e.target.value as ProductStatus | '')}
        >
      ...
      {canWrite && selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-indigo-500/40 bg-indigo-500/5 px-3 py-2">
          ...
          <Button
            size="sm"
            variant="secondary"
            disabled={!bulkStatus || bulk.saving}
            onClick={() => runBulk('setStatus', bulkStatus as ProductStatus)}
          >
            Apply
          </Button>
          <Button
      ...
      <ConfirmDialog
        ...
        message={
          <>
            Delete <span className="font-medium text-text">{toDelete?.name}</span>? This can't be
            undone.
          </>
        }
      />
      ...
      <ConfirmDialog
        ...
        message={
          <>
            Delete <span className="font-medium text-text">{selected.size}</span> selected products?
